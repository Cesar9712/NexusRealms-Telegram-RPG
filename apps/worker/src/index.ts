...
async function materializeDueNotifications(){
 await sql.begin(async tx=>{
  ...
  for(const b of due){const dedupe=`bastion:${b.character_id}:${b.building_code}:${new Date(b.upgrade_finishes_at).getTime()}`;await tx`insert into game.player_notifications(player_id,kind,payload,dedupe_key) values(${b.player_id},'bastion_complete',${tx.json({building:b.building_code,name:b.name_es,level:Number(b.level)+1})},${dedupe}) on conflict do nothing`;await tx`update game.bastion_buildings set level=level+1,upgrade_started_at=null,upgrade_finishes_at=null,updated_at=now() where character_id=${b.character_id} and building_code=${b.building_code}`;}
  const crafts=await tx`select * from game.v_finished_crafting_notifications`;
  for(const j of crafts)await tx`insert into game.player_notifications(player_id,kind,payload,dedupe_key) values(${j.player_id},'craft_ready',${tx.json({jobId:j.job_id,recipeId:j.recipe_id})},${j.dedupe_key}) on conflict do nothing`;
  ...
  for(const e of energy){const day=new Date().toISOString().slice(0,10);await tx`insert into game.player_notifications(player_id,kind,payload,dedupe_key) values(${e.player_id},'energy_full','{}',${`energy:${e.character_id}:${day}`}) on conflict do nothing`;await tx`update game.characters set energy=energy_max,energy_regen_anchor=now(),updated_at=now() where id=${e.character_id}`;}
 });
}
