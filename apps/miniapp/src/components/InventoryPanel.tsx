...
export function InventoryPanel({active,snapshot,onClose,onSnapshot}:{active:string;snapshot:Snapshot;onClose:()=>void;onSnapshot:(s:Snapshot)=>void}){
 ...
 const materials=snapshot.resources.filter(r=>!['gold','crystals','premium_credits','clan_coins'].includes(r.resource_code));
 ...
 return <section className="module-overlay inventory-premium-overlay" aria-label="Inventario">
  ...
  {tab==='materials'&&<div className="resource-vault">{materials.map(r=><article key={r.resource_code}><Package/><span>{r.resource_code.replaceAll('_',' ')}</span><b>{Number(r.amount).toLocaleString()}</b></article>)}</div>}
